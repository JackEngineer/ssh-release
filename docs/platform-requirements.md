...
- 远端可运行 `sha256sum` 或 `shasum`，用于发布后校验远端 `manifest.json`。
- 远端最好可运行 `tar`；如果远端 `tar` 不可用且 `deploy.fallbackToFileUpload` 为 `true`，会回退逐文件上传。

## macOS

macOS 默认通常已有 `ssh`、`scp` 和 `tar`。
...
Windows 可以运行 Node.js 版本的 CLI，但发布依赖 `ssh`、`scp` 和 `tar`。建议在以下环境之一运行：

- GitHub Actions 的 `ubuntu-latest` runner。
- WSL2。
- Git Bash 或其他包含 OpenSSH 和 tar 的环境。
...
`doctor` 会检查本地 `tar`、`ssh`、`scp`，使用密码登录时还会检查 `sshpass`。本地命令缺失时，先补齐依赖或改用私钥登录。
...
如果提示远端缺少 `sha256sum` 或 `shasum`，需要在服务器上安装其中一个命令，否则发布后无法校验 `manifest.json`。
